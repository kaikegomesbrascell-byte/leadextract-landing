...
export const CheckoutModal = ({ open, onOpenChange }: CheckoutModalProps) => {
  ...
  const normalizeQrCode = (code: string) => {
    ...
    // Já vem como data URI (PNG, SVG, etc) -> preserva
    if (/^data:image\/[a-zA-Z+-.]+;base64,/.test(code)) {
      return code;
    }

    // Se tiver apenas base64, converte para PNG data URI.
    if (/^[A-Za-z0-9+/=]+$/.test(code)) {
      return `data:image/png;base64,${code}`;
    }

    // Se for texto (payload brcode) ou outra coisa, tenta colocar como raw data image
    return `data:image/png;base64,${code}`;
  };

  const persistState = (nextStep: 1 | 2 | 3, nextPixData: { qrCode: string; pixCode: string } | null, nextFormData: CheckoutFormData, nextMethod: "pix") => {
  ...
  const handlePaymentMethodSelect = async (method: "pix") => {
    ...
    if (method === "pix") {
      ...
      try {
        ...
        const response = await fetch(`${apiBaseUrl}/api/payment/pix`, {
            method: "POST",
            headers: {
