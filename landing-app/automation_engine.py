...
class GoogleMapsAutomation:
    ...
    async def buscar_empresas(
        ...
        Returns:
            ...
            Cada dicionário contém: nome, telefone, site, nota, comentarios, endereco
        ...
        try:
            ...
            for index, item in enumerate(result_items[:items_to_process]):
                ...
                try:
                    ...
                    # Aguardar delay humano antes de extrair dados (OTIMIZADO)
                    await self.aplicar_delay_humano(0.5, 1.0)
        ...
        try:
            ...
            while scroll_count < max_scrolls:
                ...
                try:
                    ...
                    # 3. Aplicar delays aleatórios entre 1-3 segundos
                    await self.aplicar_delay_humano(1.0, 3.0)
    ...
    async def extrair_dados_empresa(self, elemento) -> Dict[str, str]:
        ...
        # 9. Aplicar delay aleatório após extração (OTIMIZADO)
        await self.aplicar_delay_humano(0.3, 0.7)
